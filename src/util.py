...
def random_multiply(data):
    new_data = data.copy()
    return new_data * (0.9 + random.random() / 5.0)
...
def get_annotations(
    class_split="cycle", data_folder="datasets/icbhi/ICBHI_final_database/"
):
    if class_split == "cycle":
        filenames = [
            f.strip().split(".")[0] for f in os.listdir(data_folder) if ".wav" in f
        ]
        # print(filenames[0])
        ...
        filenames = [
            f.strip().split(".")[0] for f in os.listdir(data_folder) if ".txt" in f
        ]
        tmp = pd.read_csv(
            data_folder + "ICBHI_Challenge_diagnosis.txt",
            names=["Disease"],
            delimiter="\t",
        )
...
def get_individual_segments_librosa(
    data_folder,
    filename,
    input_sec=8,
    sample_rate=16000,
...
def get_entire_signal_librosa(
    ...
    if spectrogram:
        ...
        return pre_process_audio_mel_t(yt.squeeze(), f_max=8000, nfft=nfft, hop=hop)
...
def get_individual_cycles_librosa(
    class_split,
...
def pre_process_audio_mel_t(
    audio, sample_rate=16000, n_mels=64, f_min=50, f_max=2000, nfft=1024, hop=512
):
