...
from src.model.models_cola_clustering import Cola, ColaMD
...
# def segment_cough_phases(mel_spectrogram, hop_length=512, sr=16000, ):
#     energy = np.sum(mel_spectrogram, axis=0) #Sum up energy for every timeframe. Double check axis
#
#     threshhold= 0.3 * np.max(energy)
#     start_frame = np.where(energy > threshhold)[0][0]
#
#     peak_frame = start_frame + np.argmax(energy[start_frame:])
#
#     explosive_start = start_frame
#     explosive_end = min(peak_frame + int(0.05 * sr / hop_length), len(energy) - 1)
#
#     voiced_start = intermediate_end
#     voiced_end = len(energy) - 1
#
#     # Find where energy drops below 20% of peak
#     energy_threshold = 0.2 * energy[peak_frame]
#     for i in range(voiced_start, len(energy)):
#         if energy[i] < energy_threshold:
#             voiced_end = i
#             break
#
#     # Convert frames to seconds
#     frame_to_sec = lambda x: x * hop_length / sr
#
#     return {
#         'frames': {
#             'explosive': (explosive_start, explosive_end),
#             'intermediate': (intermediate_start, intermediate_end),
#             'voiced': (voiced_start, voiced_end)
#         },
#         'seconds': {
#             'explosive': (frame_to_sec(explosive_start), frame_to_sec(explosive_end)),
#             'intermediate': (frame_to_sec(intermediate_start), frame_to_sec(intermediate_end)),
#             'voiced': (frame_to_sec(voiced_start), frame_to_sec(voiced_end))
#         },
#         'energy': energy
#     }
#
#
# def plot_results(mel_spectrogram, results, sr=16000, hop_length=512):
#     """Plot the segmentation results"""
#
#     fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 6))
#
#     # Time axis
#     time = np.arange(mel_spectrogram.shape[1]) * hop_length / sr
#
#     # Plot mel spectrogram
#     ax1.imshow(mel_spectrogram, aspect='auto', origin='lower', extent=[0, time[-1], 0, mel_spectrogram.shape[0]])
#     ax1.set_ylabel('Mel Bands')
#     ax1.set_title('Mel Spectrogram with Cough Phases')
#
#     # Add phase boundaries
#     colors = ['red', 'orange', 'green']
#     phases = ['explosive', 'intermediate', 'voiced']
#
#     for i, phase in enumerate(phases):
#         start_sec, end_sec = results['seconds'][phase]
#         ax1.axvline(start_sec, color=colors[i], linestyle='--', alpha=0.8)
#         ax1.axvline(end_sec, color=colors[i], linestyle='--', alpha=0.8)
#         ax1.text((start_sec + end_sec) / 2, mel_spectrogram.shape[0] * 0.9,
#                  phase.title(), ha='center', color=colors[i], fontweight='bold')
#
#     # Plot energy
#     ax2.plot(time, results['energy'], 'b-', linewidth=2)
...
#     for i, phase in enumerate(phases):
#         start_sec, end_sec = results['seconds'][phase]
#         ax2.axvspan(start_sec, end_sec, alpha=0.3, color=colors[i], label=phase.title())
#
#     ax2.legend()
#     plt.tight_layout()
#     plt.show()
...
class AudioDataset(torch.utils.data.Dataset):
    def __init__(self, data, max_len=200, spec_augment=True, augment=True, from_npy=False, labels=None, method="cola",
        ...
        if self.spec_augment:
            self.spec_augment_op = SpecAugment(W=50, time_mask=20, freq_mask=20)
        ...
        if self.method == "cola":
            ...
            if self.positive_pair_method == "crop":
                ...
                if time_len > self.max_len:
                    ...
                    x_cropped = np.pad(x, ((0, padding_needed), (0, 0)), mode='constant')
                else:
                    x_cropped = x
...
def train_multiple_data(title, data_source={"covidbreath": 251},  strategy="crop", dim_fea=1280, dim_hidden=1280, dim_out=512, encoder="efficientnet", n_epoches=512, training_method="cola"):
    print(data_source)
    ...
    for dt, max_len in data_source.items():
        ...
        train_data = AudioDataset(
            train,spec_augment=True, augment=False, from_npy=True, max_len=max_len, method=method, positive_pair_method=strategy)
        val_data = AudioDataset(
            test,spec_augment=True, augment=False, from_npy=True, max_len=max_len, method=method, positive_pair_method=strategy)
    ...
    trainer = pl.Trainer(
        ...
        log_every_n_steps=20  # Or any value <= 35
...
if __name__ == "__main__":
    ...
    train_multiple_data(args.title, data_source=data_source, dim_hidden=args.dim_hidden,
                        dim_out=args.dim_out, encoder=args.encoder, n_epoches=args.epoches, training_method=args.method, strategy=args.strategy)
