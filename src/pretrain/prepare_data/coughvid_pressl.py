...
def preprocess_spectrogram_SSL(modality="breath", input_sec=8):
    ...
    train_uids = np.load(
        "datasets/coughvid/coughvid__train_uuids.npy", allow_pickle=True
    ).tolist()
    val_uids = np.load(
        "datasets/coughvid/coughvid__val_uuids.npy", allow_pickle=True
    ).tolist()
    ...
    os.makedirs("datasets/coughvid/entire_spec_npy", exist_ok=True)
