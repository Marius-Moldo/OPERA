...
def finetune_coughCOPD(n_cls=2, pretrain="operaCE", l2_strength=1e-1, epochs=64, batch_size=64, lr=1e-4,
                          head="linear", feat_dim=1280):
    print("*" * 48)
    print("training dataset coughCOPD disease from model pretrained on", pretrain, "with l2_strength", l2_strength, "lr",
          lr, "head", head)
    ...
    if pretrain == "audiomae":
        ...
        if not os.path.exists(feature_dir + "fbank_audiomae.npy"):
            ...
            x_data = []
            for audio_file in sound_dir_loc:
                data = get_split_signal_fbank_pad("", audio_file[:-4], spectrogram=True, input_sec=10, trim_tail=False)[
                    0]
                # print(data.shape)
                x_data.append(data)
            x_data = np.array(x_data)
        ...
        model = AudioClassifierAudioMAE(net=net, head=head, classes=n_cls, lr=lr, l2_strength=l2_strength,
                                        feat_dim=feat_dim)
        ...
        clap_model = CLAP(version='2022', use_cuda=True)
        net = clap_model.clap.audio_encoder
        model = AudioClassifierCLAP(net=net, head=head, classes=n_cls, lr=lr, l2_strength=l2_strength,
                                    feat_dim=feat_dim)
        from_audio = True
        ...
        if not os.path.exists(feature_dir + "spectrogram_pad8.npy"):
            ...
            x_data = []
            for audio_file in sound_dir_loc:
                data = get_split_signal_librosa("", audio_file[:-4], spectrogram=True, input_sec=8.18, trim_tail=False)[
                    0]
                # print(data.shape)
                x_data.append(data)
            x_data = np.array(x_data)
        ...
        if "mae" in pretrain or "GT" in pretrain:
            model = AudioClassifierAudioMAE(net=pretrained_model, classes=n_cls, lr=lr, l2_strength=l2_strength,
                                            feat_dim=feat_dim)
        else:
            freeze_encoder = "early" if pretrain == "operaCE" else "none"
            net = pretrained_model.encoder
            model = AudioClassifier(net=net, head=head, classes=n_cls, lr=lr, l2_strength=l2_strength,
                                    feat_dim=feat_dim, freeze_encoder=freeze_encoder)
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="valid_auc", mode="max", dirpath="cks/finetune/icbhi/",
        filename="_".join(["finetuning", head, pretrain, str(batch_size), str(lr), str(epochs),
    ...
    test_res = trainer.test(dataloaders=test_loader)
    auc = test_res[0]["test_auc"]
    print("finished training dataset icbhi disease from model pretrained on", pretrain, "with l2_strength", l2_strength,
          "lr", lr, "head", head)
    return auc
...
if __name__ == "__main__":
    ...
    if not args.LOOCV:
        ...
        for seed in range(args.n_run):
            ...
            elif args.task == "coughCOPD":
                auc = finetune_coughCOPD(pretrain=args.pretrain, epochs=20, l2_strength=1e-1, feat_dim=args.dim)
