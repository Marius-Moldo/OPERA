    ...
    for modality in [
        ...
        for uuid, row in tqdm(df.iterrows(), total=df.shape[0]):
            ...
            files = gb.glob(
                "/".join(
                    [
                        "datasets/Coswara-Data/Extracted_data",
                        "*",
                        uuid,
                        modality + ".wav",
                    ]
...
def extract_and_save_embeddings(
    feature, modality, label="sex", input_sec=8, dim=1280, output_file_name="operaCE"
):
    ...
    feature += str(dim)
    np.save(
        feature_dir + output_file_name + "_feature_{}_{}.npy".format(modality, label),
